...
const HeroContainerComponent = props => {
  ...
  if (themeOverride && typeof themeOverride === 'object') {
    Object.keys(themeOverride).forEach(
      k => themeOverride[k] == null && delete themeOverride[k]
    )
  }
    ...
    <HeroContainer theme={heroTheme} image={backgroundImage}>
      <HeroContentContainer theme={heroTheme}>
        <HeroImageTextContainer>
          ...
          {subtitle && !title && (
            <HeroSubTitle theme={heroTheme}> {subtitle} </HeroSubTitle>
          )}
...
const HeroTitle = styled.h1`
  color: ${({ theme, color }) =>
    color || (theme.theme === 'light' ? theme.black : theme.white)};
