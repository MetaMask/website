...
export const fetchContentfulData = (dataType) => (id, host = CONTENTFUL_HOST) => {
  if( dataType !== "module" && dataType !== "asset" ) {
    return {
      error: {
        message: "Can only retrieve `module` and `asset` data types from Contentful"
      }
    }
  }
  ...
  const resource_url = `/spaces/${CONTENTFUL_SPACE_ID}`+
    `/environments/${CONTENTFUL_ENVIRONMENT}` +
    `/${resource}/${id}?access_token=${CONTENTFUL_API_KEY}`;

  return axios.get("https://" + host + resource_url)
    .then((result) => {
      if(result.error || !result.data) {
        return {
          error: result.error ||
            { message: `No data returned for Contentful ${dataType} with id: ` + id } 
        };
      }
...
const handleModuleResponse = (response) => {
  ...
  const internal = { type: `Contentful${type.charAt(0).toUpperCase() + type.slice(1)}` };
...
const getNestedModules = (moduleConfig) => {
  if(moduleConfig) {
    const resolvedModuleFields = mapValues(moduleConfig, (field, i) => {
      ...
      if(Array.isArray(field)) return field.map((mod) => getNestedModules({ mod }));

      // if field contains Contentful metadata then retrieve module/asset ID and call API again
      if(!field.sys) {
        return field;
      } else {
        const { sys: { linkType, id } } = field;
